...
%% 
load('SPMap_features.mat'); % Loading SP map feautures extracted from MNI and CHUGA
addpath('functions');
addpath('demo_data')
...
cov_mni = covariates(centers == 1,:)/1000; % Converting fro mm^3 to cm^3

% Obtaining CHUGA covariates
label_chuga = features(centers == 2,5);
X_chuga = features(centers == 2,[1 2 4]);
cov_chuga = covariates(centers == 2,:)/1000; % Converting fro mm^3 to cm^3
...
% Figure 8a: Comparing within the first cluster and across surgical outcome
clusterID = 1; 
...
% Figure 8b: Comparing across clusters
Z = cov_mni(:,1);
g = idx_test_mni;  
